...
export default function Verify() {
  ...
  const { profile, fetchProfile, user, authRedirectPath, setAuthRedirectPath } =
    useAuthStore();
  ...
  useEffect(() => {
    ...
    if (user && verified) {
      if (profile) {
        if (redirectPath) {
          ...
          if (authRedirectPath) {
            router.replace(authRedirectPath as RelativePathString);
    ...
    setVerified(false);
  }, [profile, user, verified, redirectPath]);
